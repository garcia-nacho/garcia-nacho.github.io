...
We control the cart by moving it to the right or the left so the pole, which is attached to the cart, will move. The goal of the environment is to maintain the pole in equilibrium preventing it to reach a certain inclination.    
...
<code>
  # The dataframe dfEyes will record: rewards, observations, status, actions, etc...<br>
  ...
  for (j in 1:max_steps) {<br>
    ...
    # The action is randomly selected from possible options<br>
