...
<code>
  ...
  #After reseting it we obstain the first 4 observations before we take any decision<br>
...
weightsN1 <-  matrix(data = runif(((Observations)*Neurons), min = -1, max = 1), ncol = Observations, nrow = Neurons)<br>
...
<code>
    ...
    Input$Out[i] <- round(sigmoid(sum(Neuron.layer*weightsN2), method = "logistic"))<br>
}<br>
</code>
...
for (k in 1:(rounds)) {
                    ...
                    <br>
                    <code>
  # dfGod is the dataframe where all the observation for all iterations are stored<br>
