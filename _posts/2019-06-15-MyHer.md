...
To do so we are going to use again the SNP the rs10000008 an example. Here it is the list of all frequencies associated with the variant form (T) of that  SNP:  
{: style="text-align: justify"}
...
First of all, the frequencies provided by the [1000genomes consortium](http://www.internationalgenome.org/) only account for the overall probability of having a variant (T),  so we need to transform those probabilities into the probabilities of having each on of the three possible genotypes (CC, TC, and TT) and to do that we use these formulas:
{: style="text-align: justify"}
...
By aplying the formulas we obtain these probabilities for TT:
{: style="text-align: justify"}
...
So the frequentist analysis tells us that if someone has a TT in the SNP rs10000008 his or her chances of being African are around 97% 
{: style="text-align: justify"}
 ...
 **P(B|A)** is the conditional probability of B occurring given the fact A
 i.e. The probability of rs10000008 equal to TT among the African population
 {: style="text-align: justify"}
 
 **p(B)** is the probability of B occurring
 i.e The overall probability of having certain variant (rs10000008 equal to TT)
 {: style="text-align: justify"}
...
EAS: 27%  
{: style="text-align: justify"}
What using P(A) = 0.17 actually means is that we don't have any preconception about the result and since the 17% percent of the world population is African we assume that the probability of being African is 17% before obtaining the result. The powerful of Bayesian analyses is that we could modify our priors. Now imagine that we are taking the sample in the middle of Kinshasa, our prior for being African would be way higher. We could say that the Bayesian analysis computes how our beliefs (being African) should change after obtaining certain evidence (having certain variant).
{: style="text-align: justify"}

We plug in this  values

 P(B|A) = 8.1%  
 P(B) = 1.44% (The ponderated percentage for the variant)  
 P(A) = 17%  
 
And the calculation gives us 95.63%, which is pretty close to the result obtained using by frequentist approach.
 {: style="text-align: justify"}
...
When two events are not ligated (I will come to point this later) we can assume that:
{: style="text-align: justify"}
