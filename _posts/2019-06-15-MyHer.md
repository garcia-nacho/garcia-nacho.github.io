---
layout: post
title:  Using ML to analyze your genetic data.
...
Now that we know how the SNPs can help us to track our origins down; however, we still don't know how the percentages provided by MyHeritage were obtained and what they really mean. Are they the probabilities of her belonging to each one of those populations? Are they the percentage of DNA associated certain ethnicities? Are they something else? I don't know and I am sure they are not going to disclose it. So we will need to perform our own analysis and compare our results with the report, but before doing it, we need to establish the statistical framework of the analyses. 
{: style="text-align: justify"}
...
To do so we are going to use again the SNP the rs10000008 and the list of all frequencies associated with the variant form (T) which are these:  
{: style="text-align: justify"}
...
First of all, the frequencies provided by the [1000genomes consortium](http://www.internationalgenome.org/) only account for the overall probability of having a variant (T),  so we need to transform those probabilities into the probabilities of having the three possible genotypes (CC, TC, and TT) and to do that we use these formulas:
{: style="text-align: justify"}

 ${P(rs10000008=TT \cap \;  ETH=AFR) = (rs10000008.AFR.Freq)^2 }$  
 ${P(rs10000008=CC \cap \; ETH=AFR) = (1 - rs10000008.AFR.Freq)^2 }$  
${P(rs10000008=TC \; OR \; rs10000008=CT \cap ETH=AFR) = 2  \; rs10000008.AFR.Freq \; (1-rs10000008.AFR.Freq)}$  


By aplying the formulas we obtain these probabilities for CC:
{: style="text-align: justify"}
...
So if we relativize the probabilities to add 100% (because we are 100% sure -in theory-) that the genotype is CC, we have the following percentages:
{: style="text-align: justify"}
...
So the frequentist analysis tells us that if someone has a CC in the SNP rs10000008 his or her chances of being African are around 97% 
{: style="text-align: justify"}

*Note:* To achieve a better accuracy we have to account for the sizes of the populations by multiplying those percentages by the ratio of the total population from that ethnicity.
{: style="text-align: justify"}

Now, let's see now what the Bayesian analysis tells us: Bayesian analyses rely on the Bayes' theorem: 
{: style="text-align: justify"}
 ...
 **P(A|B)** is the conditional probability of A occurring given the fact B 
 i.e: The probability of being African if you have the SNP rs10000008 equal to CC.
  {: style="text-align: justify"}
 
 **P(B|A)** is the conditional probability of B occurring given the fact A
 i.e. The probability of rs10000008 equal to CC among the African population
 {: style="text-align: justify"}
 
 **p(B)** is the probability of B occurring
 i.e The overall probability of having certain variant (rs10000008 equal to CC)
 {: style="text-align: justify"}
...
Before plugging the data that we know, we have to define the distribution of populations around the globe, because we will use them as *priors* for our analysis: 
{: style="text-align: justify"}
AFR: 17%  
EUR: 10%  
SAS: 31%  
...
EAS: 27%  
{: style="text-align: justify"}
What that actually means is that we don't have any preconception about the result and since the 17% percent of the world population is African we assume that the probability of being African is 17% before obtaining the result. The powerful of Bayesian analyses is that we could modify our priors. Now imagine that we are taking the sample in the middle of Kinshasa, our prior for being African would be way higher. We could say that the Bayesian analysis computes how our beliefs should change after obtaining certain evidence.
{: style="text-align: justify"}
 ...
 The calculation of P(A|B) results in 95.63% which is pretty close to the result obtained using by frequentist approach.
 {: style="text-align: justify"}
 
Well... We have used only one SNP to calculate the probability of belonging to a particular ethnic group. What are the rest of the SNPs used for then??!
 {: style="text-align: justify"}

To make things more interesting, we could try to combine all possible SNPs to give a more accurate probability; however, here is when both strategies fail...
 {: style="text-align: justify"}

**The frequentist failure:** 
To calculate the combined probability one could think that the multiplication of frequencies for all SNPs would do the trick, but it obviously does not: The value gets smaller and smaller as it is multiplied by numbers lower than zero. That is because the real frequency of the combination of both is unknown and impossible to calculate it with the data that we have.
{: style="text-align: justify"}
...
When two events are not ligated (we will come to this later) we can assume that:
{: style="text-align: justify"}
...
Thus we obtain P(B \cap C) that we can substitute to end up with the final equation:
{: style="text-align: justify"}
...
In this case again we face a similar problem as in the frequentist approach, the numerator gets smaller and smaller with each SNP added, however in theory it shouldn't be a problem because the denominator decreases in a similar range. However, in terms of computability it becomes impracticable after few hundreds of SNPs because even a machine can't compute such small numbers.
{: style="text-align: justify"}

Anyway, using hundreds of SNPs has to be better than using just one in order to obtain an accurate probability, right? Let's compute it using R...{: style="text-align: justify"}

### Analysis of populations using Bayes' theorem in R

First, we need to perform some operations outside R since we need to get all the data that we need from a web server.
{: style="text-align: justify"}

The server that we are going to use is [www.snp.nexus.org](www.snp-nexus.org), where you can just send a list of SNPs and obtain several files containing information regarding them back; unfortunately, they only allow up to 100K SNPs per query, so we have to split our more than 600K SNPs into 7 files using R.
{: style="text-align: justify"}

First, we load the raw data provided by MyHeritage and based on the number of SNPs we generate the files cotaining up to 100K SNPs in each.
{: style="text-align: justify"}
...
Then, we upload them one by one into the server. The query takes around 14 hours to be processed, so it is a good idea to provide an email in the online form so you can receive an email when the data is ready to download.
{: style="text-align: justify"}
...
Note: There are many many ways of doing this process, here I'm just broadly describing the strategy that I used.
{: style="text-align: justify"}

Now we can start importing the files into R:
{: style="text-align: justify"}

<pre><code>#Populations loading 
...
Now let's assign the *priors* for each population and how they are distributed worldwide.
{: style="text-align: justify"}
...
Now we can clean the data by removing duplicated elements and the SNPs in our results that were not available at www.snp.nexus.org](www.snp-nexus.org):
{: style="text-align: justify"}
...
And calculate some probabilities (For the sake of simplicity I only show the calculations for the African population, but you can find the complete script [here](https://github.com/garcia-nacho/MachineLearning/tree/master/GeneticAnalysis))
{: style="text-align: justify"}
...
Then, we subset the dataset to separate the SNPs into heterozygous, homozygous reference and homozygous variants.
{: style="text-align: justify"}
...
and we specify the priors again and the number of SNPs to test. Note: I included the priors here again for testing purposes, alternatively, it would be very easy to recall the previous function and remap the values of the vector <code>priors1000genomes</code> into the new variables.
{: style="text-align: justify"}
...
Wooow! Totally unexpected results, all probabilities collapse to zero after few SNPs. As the 150 SNP to analyze are random I run it again. 
{: style="text-align: justify"}

![bayes1](/images/bayes2.png)

Same results, probabilities collapse after few SNPs but with a different profile. This is clearly not the way to go because of the unpredictibility.
{: style="text-align: justify"}
...
The idea is very simple. We know the probabilities associated with each variant for every ethnicity, so it should be possible to simulate the genotype of individuals for all SNPs. Then using unsupervised clustering we can find out where our sample clusters.
{: style="text-align: justify"}

To do that we first need to simulate the results of N individuales from every population:
{: style="text-align: justify"}
...
Then we create a list with the number of SNPs that we want to use and a matrix containing the different individuals in rows, being the first one our sample.
...
And we explore the group associated with the sample:
{: style="text-align: justify"}
...
Here we have a better view of what is going on and how different groups correlate to each other, for instance, we can appreciate that the African popu1lation is more isolated (and probably the ancient).
{: style="text-align: justify"}

As you can see, machine-learning strategies such as k-means perform much better than classical statistical approaches for genetic analyses. Of course, more SNPs would require longer computational times as well as a larger amount of simulations but nothing a semi-decent computer can't do in a few minutes, giving you more accurate results.
Although I don't have a formal mathematical explanation for the difference in the performance I suspect that it might be related to the dependency of the variables. The Bayesian analyses as I intentionally mentioned assumes that the values are independent. However human SNPs are usually linked, that means that some SNPs are inherited together so there is no independence. This groups of SNPs are what geneticists call [haplotypes](https://en.wikipedia.org/wiki/Haplotype).
{: style="text-align: justify"}

### Ethnicity calculation by DNA fragments.
So far we have seen how to calculate the overall ethnicity of a person based on the results obtained from MyHeritage; however, in this section, we are going to split the whole genome in fragments and we are going to see if different fragments have different ethnicities. The rationale behind this analysis is that regions close in the genome tend to remain intact after several generations (they are linked). We could then say that the more fragmented the genome is (in terms of ethnicity) the more distant in time the sample is to that ancestor/s who carried that DNA fragment because of the [meiotic recombination](https://cshperspectives.cshlp.org/content/7/12/a016618.full).
...
Then, to *chop* the chromosomes we need an additional file that you can find [here](/images/genome.txt.csv). That file contains information regarding the chromosome size and the position of the [centromere](https://en.wikipedia.org/wiki/Centromere). Using that information we can divide the chromosome into pieces of a particular size, stated in the variable <code>genomic.size</code>. For simplicity, we also rename chromosomes X and Y to 23 and 24.
{: style="text-align: justify"}
...
We create some empty columns in the data frame that will contain all the information once we fill the array with a loop.
{: style="text-align: justify"}
...
As you can see it is much more efficient to have 10 cores working in the same process that 1 core doing all the task, and indeed it divides the computing time by 10.
{: style="text-align: justify"}

As a rule of thumb, when you are doing parallel computing you always divide the larger tasks to be performed, that usually means to replace the external loop (for/while/etc) by the parallelizing function <code>foreach()%dopar%</code>
{: style="text-align: justify"}
...
{: style="text-align: justify"}
I have devised a genetic score in which we calculate the difference in the probability of having certain SNP between a population and the rest of the populations. If that mutation is more likely to happen in the selected population the value is positive and negative if it is less likely. The higher the value (positive or negative) the higher the importance of that SNP to calculate the score, then we only have to calculate the mean of the scores for every SNP in the region that we are investigating and repeat it for every population, the population with the higher positive score is the one assigned to that region.
{: style="text-align: justify"}
...
So we only need to call it during each interation.
...
After plotting the results (that I am not showing for privacy reasons) I have found that most of the predicted regions in the chromosomes are quite simmilar; however, the unsupervised clustering approach seems to be more precise. 
{: style="text-align: justify"}
...
{: style="text-align: justify"}
In the future, I will write about how to analyze predisposition to certain diseases based on the data provided. I will also include a more complete genetic study such as HapMap in which there are many more ethnicities than in the 1000genomes. If you are looking forward to squeezing your data, stay tuned!   
{: style="text-align: justify"}
