...
## Introduction
We humans learned how to use drugs many thousands of years ago. The shaman of the tribe knew which roots and plants had healing effects on sick people. Of course, he didn't know of anything about chemistry or why they work, but that was the pre-scientific era, so who cared?! 
Slowly, mankind became better at using drugs, but only as a consequence of a long trial and error process. It was not until the scientific revolution when we started an active and systematic search for new medicines which together with the advances in chemistry and biology, lead us to the golden age of medicine in the middle of the 20th century when there was an explosion in the number of antibiotics, anticancer or antifungal treatment. It seemed that in a few years we would be able to cure anything. 
Unfortunately, that was not the case, the infections are becoming resistant to the current treatments and cancer treatments are ineffective and unspecific. Unfortunately, the drug-discovery process is slow and expensive and since the number of *drugs-to-be-found* is finite it is becoming more and more difficult to find unknown bioactive-chemicals, making the process more and more difficult.
{: style="text-align: justify"}
We clearly need new approaches to speed up the process of drug discovery and in this series of post, I will show you how to use ML and AI to reach that goal. Through several posts, we will be dissecting some *state of the art* ML-driven strategies that you can use to find new drugs and I hope that at the end of the series you will have the knowledge and the tools to continue the journey on your own.  
<!--more-->

## The rationale behind using ML to find drugs
When we create a ML model we are creating an algorithm that learns how to approximate a function *f(x)=y*. In our case the function that we want to approximate is simple: *f(drug)=activity* the hypothesis to test is that *"if we can learn (or approximate) the f(drug)=activity function we can instantly predict the activity of a drug without testing it in the lab"*. That would save thousands of hours and tons of money in the drug-discovery process. At the end of this post, you will see by yourself how you can screen thousands of *unknown* drugs in minutes.
{: style="text-align: justify"}

The first thing that we need to do if we want to approximate the function *f(drug)=activity* is to parameterize the two terms of the function. The parameterization of the *activity* term is easy because we can measure the activity, it can be EC50 which is the concentration at which the drug has the half of the maximum achievable effect, it can be in the form of pKi which is a measurement of the strength of a drug-protein interaction or it can be in whatever units. It can be continuous ranging between two values or discrete, defining two or more classes (no-activity, moderate-activity, high-activity). It doesn't matter as long as you can measure it and it is useful for the inferences you want to make about other unknown drugs. 
